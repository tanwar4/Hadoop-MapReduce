...
public class CoarseLock {
	...
	static HashMap<String,ArrayList<Double>> accumulation_data;
	...
	public static void main(String[] args) {
					...
					public void run(){	
						for(String str:partition.get(index)){
							...
							if(tokens[2].equalsIgnoreCase("TMAX") && !(tokens[3].isEmpty())){  //extract only records which has T-MAX
								...
								//Locking the shared data structure ; this will lock the entire datastructure so other threads will wait
								synchronized (accumulation_data) {
									if(version == 1){
										fibonacci(17);
									}
		...
		System.out.println(" Coarse Min:"+min +" Max: "+max +" Avg"+sum/10);
	}
